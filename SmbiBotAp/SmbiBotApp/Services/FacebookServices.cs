...
namespace SmbiBotApp.Services
{
    ...
    public class FacebookServices
    {
        ...
        public async Task<dynamic> GetUser(string token,string id)
        {
            string[] arr = new string[7];
            string[] mis = new string[7];
            ...
            for (int i = 0; i <= 6; i++)
            {
                ...
                if (!exi)
                {
                    mis[i] = Enum.GetName(typeof(MessagesController.user_profile), i);
                    arr[i] = "Not Specified";
                }
                else
                {
            ...
            // string[] arr = ((IEnumerable)val).Cast<object>().Select(x => x.ToString()).ToArray();
            //  object val2 = user.GetType().GetProperty("Keys").GetValue(user, null);
            //  string[] arr2 = ((IEnumerable)val2).Cast<object>().Select(x => x.ToString()).ToArray();
